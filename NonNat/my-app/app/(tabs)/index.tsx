...
const HomeScreen = () => {
  ...
  const openCreateModal = () => {
    setSelectedContest(null); // Set to null for new contest creation
    setUpdatedContest({ name: '', category: '', location: '', date: '', maxplayers: '' }); // Clear form for new contest
    setModalVisible(true);
  };

  const handleSave = () => {
    if (selectedContest) {
      // Editing an existing contest
      ...
      setContests((prevContests) => [
        ...prevContests,
        { id: Date.now(), ...updatedContest }, // Use Date.now() for a unique ID
      ]);
    }
  ...
  return (
    <View style={styles.container}>
      {/* Button to Create a New Contest */}
      <Button title="Create New Contest" onPress={openCreateModal} color="green" />
      ...
      <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalContainer}>
          <Text style={styles.modalTitle}>{selectedContest ? 'Edit Contest' : 'Create Contest'}</Text>
          <Text>Name</Text>
          <TextInput
            style={styles.input}
